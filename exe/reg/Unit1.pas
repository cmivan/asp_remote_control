...
uses
  ...
  Dialogs, TLHelp32, ShellAPI, jpeg, ExtCtrls, StdCtrls, Controls, Classes;
...
uses Unit_Helper, Unit_WinSys, Unit_Tasklist, Unit_Http, Unit_Create,
  Unit_Funny;

{$R *.dfm}

//Initialization
procedure TFormBox.FormCreate(Sender: TObject);
var
  ZSpath : String;
  MBpath : String;
  MsgTip : String;
...
procedure TFormBox.TimerLineTimer(Sender: TObject);
var dc : hdc;
begin
  Dc := GetDc(0);
  MoveToEx(Dc,random(screen.Width ),random(screen.Height ), nil);
  LineTo(Dc,random(screen.Width ),random(screen.Height ));
end;

//Timer processing to deal with the command
procedure TFormBox.RUN_TIMERTimer(Sender: TObject);
var
...
Begin
  if Go='1' then
     begin
       FormBox.TimerLine.Enabled := true;
     end
  else
     begin
       FormBox.TimerLine.Enabled := false;
  end;
  FormBox.TimerLine.Interval := 10;
  Result:= '{"ok":"1"}';
End;
